...
from base.history_retriever import get_history
...
# # The response is a streaming output, you need to process it accordingly
# full_response = ""
# for event in response['body']:
#     if 'Payload' in event:
#         chunk = event['Payload'].decode('utf-8')
#         full_response += chunk


def project_generator(session_id, **kwargs):
    load_dotenv()
    client = boto3.client('bedrock-runtime', region_name='us-west-2', aws_access_key_id=os.getenv("AWS_ACCESS_KEY"), aws_secret_access_key = os.getenv("AWS_SECRET_ACCESS_KEY"))
